...
static void thread_shutdown() {
     ...
     */
	Console::puts("Scheduler called thread_shutdown()\n");
	SYSTEM_SCHEDULER->terminate(Thread::CurrentThread());
	//SYSTEM_SCHEDULER->terminate(current_thread);

	delete current_thread;
	
	// However, we only want to continue if we have more threads in our ready list.
	if (SYSTEM_SCHEDULER->thread_list->thread != NULL){	
		Console::puts("Yielding after deleting current_thread..\n");
		SYSTEM_SCHEDULER->yield();
	}
    else {
		Console::puts("No more threads in ready queue!\n");
	}
}
